  ...
  <div class="container mt-4" v-if="isInstructor">
    ...
    <div class="files-section mt-4">
      ...
      <div class="form-group">
        ...
        <select
          id="view-week"
          v-model="selectedViewWeek"
          class="form-control"
          @change="fetchUploadedFiles"
        >
          <option v-for="week in weeks" :key="week" :value="week">
            Week {{ week }}
          </option>
      ...
      <ul v-if="uploadedFiles.length" class="list-group">
        ...
        >
          ...
          <div>
            <button
              @click="viewFile(pdf.file_url)"
              class="btn btn-success btn-sm mr-2"
            >
              View
            </button>
            |
            <button @click="deleteFile(pdf.id)" class="btn btn-danger btn-sm">
              Delete
            </button>
          </div>
        </li>
      ...
      <p v-else class="text-muted">
        No files uploaded yet for Week {{ selectedViewWeek }}.
      </p>
    </div>
...
export default {
  ...
  methods: {
    ...
    async uploadFile() {
      ...
      try {
        const response = await axios.post(
          "https://seek-protal-gzcname5gsghfhhj.centralindia-01.azurewebsites.net/api/supplementary",
          formData,
          {
            headers: {
    ...
    async fetchUploadedFiles() {
      try {
        ...
        const response = await axios.get(
          `https://seek-protal-gzcname5gsghfhhj.centralindia-01.azurewebsites.net/api/supplementary/1/${this.selectedViewWeek}`,
          {
            headers: {
              Authorization: `Bearer ${user.token}`,
            },
        ...
        console.log(
          `📂 Fetched files for Week ${this.selectedViewWeek}:`,
          response.data
        );
      } catch (error) {
        console.error(
    ...
    viewFile(fileUrl) {
      if (fileUrl) {
        const absoluteUrl = `https://seek-protal-gzcname5gsghfhhj.centralindia-01.azurewebsites.net${fileUrl}`;
    ...
    async deleteFile(fileId) {
      try {
        ...
        await axios.delete(
          `https://seek-protal-gzcname5gsghfhhj.centralindia-01.azurewebsites.net/api/supplementary/${fileId}`,
          {
            headers: {
              Authorization: `Bearer ${user.token}`,
            },
