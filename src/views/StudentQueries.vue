<template>
  <div class="student-queries-page">
    ...
    <div v-if="loading" class="loading">Loading your queries...</div>
...
export default {
  ...
  data() {
    return {
      ...
      error: null,
      baseURL:
        "https://seek-protal-gzcname5gsghfhhj.centralindia-01.azurewebsites.net",
      user: null,
    };
  },
  mounted() {
  ...
  methods: {
    ...
    async fetchQueries() {
      try {
        ...
        const markSeenResponse = await axios.post(
          `${this.baseURL}/api/replies/mark-seen`,
          {
        ...
        this.queries = response.data.filter(
          (query) => query.student_email === this.user.email
        );
      } catch (error) {
        console.error(
          "Error fetching queries or marking replies as seen:",
          error
        );
