<template>
  <div class="instructor-page">
    ...
    <div class="content-container">
      <div class="actions">
        <button @click="goToSupplementaryManage" class="manage-content">
          Manage Supplementary Content
        </button>
        <button @click="goToEnrolledStudents" class="manage-students">
          Manage Enrolled Students
        </button>
        <button @click="goToStudentQueries" class="manage-queries">
          Students Feedback
        </button>
      </div>
      ...
      <div v-if="students.length" class="section">
        ...
        <ul class="list-container">
          <li v-for="student in students" :key="student.id" class="list-item">
      ...
      <div v-if="assignments.length" class="section">
        ...
        <ul class="list-container">
          <li
            ...
            class="list-item"
          >
            Assignment #{{ assignment.assignment_no }} - Week
            {{ assignment.week_no }}
          </li>
        </ul>
      </div>
      ...
      <div v-if="feedbacks.length" class="section">
        ...
        <ul class="list-container">
          <li
            ...
            :key="feedback.feed_id"
            class="list-item"
          >
            {{ feedback.feed_content }} - <strong>Rating:</strong>
            {{ feedback.feed_rating }}/5
          </li>
        </ul>
      </div>
...
export default {
  ...
  methods: {
    async fetchCourses() {
      try {
        ...
        const response = await axios.get(
          `https://seek-protal-gzcname5gsghfhhj.centralindia-01.azurewebsites.net/api/course/${user.user_id}`,
          {
    ...
    async fetchStudents() {
      try {
        const response = await axios.get(
          "https://seek-protal-gzcname5gsghfhhj.centralindia-01.azurewebsites.net/api/enrollment/1",
          {
            headers: {
    ...
    async fetchAssignments() {
      try {
        const response = await axios.get(
          "https://seek-protal-gzcname5gsghfhhj.centralindia-01.azurewebsites.net/api/assignment/1",
          {
            headers: {
              Authorization: `Bearer ${
                JSON.parse(localStorage.getItem("user")).token
    ...
    async fetchFeedback() {
      try {
        const response = await axios.get(
          "https://seek-protal-gzcname5gsghfhhj.centralindia-01.azurewebsites.net/api/feedback/1",
          {
            headers: {
              Authorization: `Bearer ${
                JSON.parse(localStorage.getItem("user")).token
    ...
    async fetchWeeklyAverageScores() {
      try {
        const response = await axios.get(
          "https://seek-protal-gzcname5gsghfhhj.centralindia-01.azurewebsites.net/api/weekly-average-scores"
        );
    ...
    renderChart(labels, scores) {
      const ctx = document
        .getElementById("weeklyAverageChartInstructorView")
        .getContext("2d");
      new Chart(ctx, {
        type: "bar",
        data: {
