...
export default {
  ...
  methods: {
    async fetchStudents() {
      try {
        const response = await fetch(
          "https://seek-protal-gzcname5gsghfhhj.centralindia-01.azurewebsites.net/api/enrolled-students"
        );
        if (response.ok) {
          ...
          if (data && Array.isArray(data.students)) {
            const uniqueStudents = Array.from(
              new Map(
                data.students.map((student) => [student.user_id, student])
              ).values()
            ); // Remove duplicates
            this.students = uniqueStudents.filter(
              (student) =>
                ![
                  "instructor@ds.study.iitm.ac.in",
                  "ta@ds.study.iitm.ac.in",
    ...
    async toggleBlock(student) {
      try {
        const response = await fetch(
          `https://seek-protal-gzcname5gsghfhhj.centralindia-01.azurewebsites.net/api/user/${student.user_id}/block`,
          {
            method: "PUT",
            headers: {
        ...
        alert(
          "An error occurred while toggling the block status. Please check your network connection and try again."
        );
      }
    },
  },
